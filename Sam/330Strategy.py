    ...
    def run(self, state: TradingState):
        ...
        if 'STARFRUIT' in state.order_depths:
            ...
            if len(self.star_historical_bid_prices) > 37:
                ...
                mid_price_rolling_mean_long = pd.Series(self.star_historical_mid_prices).rolling(window=20).mean()
                mid_price_rolling_mean_short = pd.Series(self.star_historical_mid_prices).rolling(window=1).mean()

                prev_long = mid_price_rolling_mean_long.iloc[-2]
                curr_long = mid_price_rolling_mean_long.iloc[-1]
                prev_short = mid_price_rolling_mean_short.iloc[-2]
                curr_short = mid_price_rolling_mean_short.iloc[-1]

                if prev_long < prev_short and curr_long >= curr_short:
                    star_orders.append(Order('STARFRUIT', star_live_ask_price, 1))
                elif prev_long > prev_short and curr_long <= curr_short:
                    star_orders.append(Order('STARFRUIT', star_live_bid_price, -1))
