    ...
    def run(self, state: TradingState):
        ...
        if 'AMETHYSTS' in state.order_depths:
            ...
            if len(self.historical_prices) > 20:
                ...
                # Trading logic with guaranteed position closing
                ...
                if not self.position_open:
                    if mid_price > latest_upper_band:
                        orders.append(Order('AMETHYSTS', best_bid_price, -1))  # Open short position
                        self.position_open = True
                        self.position_type = 'Short'
                        print('open upper')
                    elif mid_price < latest_lower_band:
                        orders.append(Order('AMETHYSTS', best_ask_price, -1))  # Open long position
                        self.position_open = True
                        self.position_type = 'Long'
                        print('open lower')
                elif self.position_open:
                    if self.position_type == 'Short' and mid_price < latest_predicted_price:
                        orders.append(Order('AMETHYSTS', best_ask_price, -1))  # Close short position
                        self.position_open = False
                        print('close upper')
                    elif self.position_type == 'Long' and mid_price > latest_predicted_price:
                        orders.append(Order('AMETHYSTS', best_bid_price, -1))  # Close long position
                        self.position_open = False
                        print('close lower')
