...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if 'STARFRUIT' in state.order_depths:
            ...
            if len(self.star_historical_bid_prices) > 20:
                ...
                if len(list(star_order_depth.sell_orders.items())) > 1:
                    star_2_live_ask_price, star_2_live_ask_volume = list(star_order_depth.sell_orders.items())[1]
                if len(list(star_order_depth.buy_orders.items())) > 1:
                    star_2_live_bid_price, star_2_live_bid_volume = list(star_order_depth.buy_orders.items())[1]
                ...
                if not self.star_position_open:
                    if star_live_bid_price > star_latest_upper_band:
                        if star_2_live_bid_price !=0 and star_2_live_bid_price > star_latest_upper_band:
                            self.star_open_order_volume = min(abs(star_2_live_bid_volume), 20)
                            star_orders.append(Order('STARFRUIT', star_2_live_bid_price, -self.star_open_order_volume))
                            print("star open upper (2)")
                            self.star_latest_price = star_2_live_bid_price
                        ...
                        if star_2_live_ask_price !=0 and star_2_live_ask_price < star_latest_lower_band:
                            self.star_open_order_volume = min(abs(star_2_live_ask_volume), 20)
                            star_orders.append(Order('STARFRUIT', star_2_live_ask_price, self.star_open_order_volume))
                            print("star open lower (2)")
                            self.star_latest_price = star_2_live_ask_price
                            self.star_position_open = True
                    ...
                    if not self.star_partially_closed:
                        if self.star_position_type == 'Short' and star_live_ask_price < star_latest_lower_band:
                            if star_2_live_ask_price !=0 and star_2_live_ask_price < star_latest_lower_band:
                                star_closing_volume = min(abs(self.star_open_order_volume), abs(star_2_live_ask_volume))
                                star_orders.append(Order('STARFRUIT', star_2_live_ask_price, star_closing_volume))
                            ...
                            if star_2_live_bid_price !=0 and star_2_live_bid_price > star_latest_upper_band:
                                star_closing_volume = min(abs(self.star_open_order_volume), abs(star_2_live_bid_volume))
                                star_orders.append(Order('STARFRUIT', star_2_live_bid_price, -star_closing_volume))
                                self.star_remaining_quantity -= star_closing_volume
                            else:
        ...
        if 'AMETHYSTS' in state.order_depths:
            ...
            if len(self.am_historical_bid_prices) > 20:
                ...
                am_lower_band = am_ask_rolling_mean - am_ask_rolling_std.values * self.am_std_multiplier
                ...
                if len(list(am_order_depth.sell_orders.items())) > 1:
                    am_2_live_ask_price, am_2_live_ask_volume = list(am_order_depth.sell_orders.items())[1]
                if len(list(am_order_depth.buy_orders.items())) > 1:
                    am_2_live_bid_price, am_2_live_bid_volume = list(am_order_depth.buy_orders.items())[1]
                ...
                if not self.am_position_open:
                    if am_live_bid_price > am_latest_upper_band:
                        if am_2_live_bid_price != 0 and am_2_live_bid_price > am_latest_upper_band:
                            self.am_open_order_volume = min(abs(am_2_live_bid_volume), 20)
                            am_orders.append(Order('AMETHYSTS', am_2_live_bid_price, -self.am_open_order_volume))
                            print("am open upper")
                            self.am_latest_price = am_2_live_bid_price
                            self.am_position_open = True
                            self.am_position_type = 'Short'
                            self.am_remaining_quantity = self.am_open_order_volume
                        else:
                            self.am_open_order_volume = min(abs(am_live_bid_volume), 20)
                        ...
                        if am_2_live_ask_price != 0 and am_2_live_ask_price < am_latest_lower_band:
                            self.am_open_order_volume = min(abs(am_2_live_ask_volume), 20)
                            am_orders.append(Order('AMETHYSTS', am_2_live_ask_price, self.am_open_order_volume))
                            print("am open lower")
                            self.am_latest_price = am_2_live_ask_price
                            self.am_position_open = True
                            self.am_position_type = 'Long'
                            self.am_remaining_quantity = self.am_open_order_volume
                        else:
                            self.am_open_order_volume = min(abs(am_live_ask_volume), 20)
                    ...
                    if not self.am_partially_closed:
                        if self.am_position_type == 'Short' and am_live_ask_price < am_latest_lower_band:
                            ...
                            if self.am_remaining_quantity > 0:
                                ...
                                self.am_partially_closed = False
                                self.am_position_open = False
                        elif self.am_position_type == 'Long' and am_live_bid_price > am_latest_upper_band:
