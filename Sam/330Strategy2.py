...
class Trader:
    def __init__(self):
        ...
        self.star_position_open_2 = False
        self.star_position_type = None
        self.star_position_type_2 = None
        ...
        self.star_remaining_quantity_2 = 0
        self.star_open_order_volume = 0
        self.star_open_order_volume_2 = 0
        self.star_partially_closed = False
        self.star_partially_closed_2 = False
        self.star_latest_price = 0
        self.star_latest_price_2 = 0
    ...
    def run(self, state: TradingState):
        if state.traderData:
            ...
            self.star_position_open_2 = saved_state.star_position_open_2
            self.star_position_type = saved_state.star_position_type
            self.star_position_type_2 = saved_state.star_position_type_2
            self.star_remaining_quantity = saved_state.star_remaining_quantity
            self.star_remaining_quantity_2 = saved_state.star_remaining_quantity_2
            ...
            self.star_open_order_volume_2 = saved_state.star_open_order_volume_2
            self.star_partially_closed = saved_state.star_partially_closed
            self.star_partially_closed_2 = saved_state.star_partially_closed_2
            self.star_latest_price = saved_state.star_latest_price
            self.star_latest_price_2 = saved_state.star_latest_price_2
        ...
        if 'STARFRUIT' in state.order_depths:
            ...
            if len(self.star_historical_bid_prices) > 37:
                ...
                if len(list(star_order_depth.sell_orders.items())) > 1:
                    star_live_ask_price_2, star_live_ask_volume_2 = list(star_order_depth.sell_orders.items())[1]
                if len(list(star_order_depth.buy_orders.items())) > 1:
                ...
                if not self.star_position_open:
                    if star_live_bid_price > star_latest_upper_band:
                        if self.star_position_type_2 == 'Short':
                            self.star_open_order_volume = min(abs(star_live_bid_volume), 10-self.star_open_order_volume_2)
                        else:
                        ...
                        if self.star_position_type_2 == 'Long':
                            self.star_open_order_volume = min(abs(star_live_ask_volume), 10-self.star_open_order_volume_2)
                        else:
                ...
                if not self.star_position_open_2:
                    if len(list(star_order_depth.buy_orders.items())) > 1:
                        if (star_live_bid_price_2 > star_latest_upper_band):
                            if self.star_position_type == 'Short':
                                self.star_open_order_volume_2 = min(abs(star_live_bid_volume_2), 20-self.star_open_order_volume)
                            else:
                                self.star_open_order_volume_2 = min(abs(star_live_bid_volume_2), 20)
                            star_orders.append(Order('STARFRUIT', star_live_bid_price_2, -self.star_open_order_volume_2))
                            print("star open upper")
                            self.star_latest_price_2 = star_live_bid_price_2
                            self.star_position_open_2 = True
                            self.star_position_type_2 = 'Short'
                            self.star_remaining_quantity_2 = self.star_open_order_volume_2
                    elif len(list(star_order_depth.sell_orders.items())) > 1:
                        if (star_live_ask_price_2 < star_latest_lower_band):
                            if self.star_position_type == 'Long':
                                self.star_open_order_volume_2 = min(abs(star_live_ask_volume_2), 20-self.star_open_order_volume)
                            else:
                                self.star_open_order_volume_2 = min(abs(star_live_ask_volume_2), 20)
                            star_orders.append(Order('STARFRUIT', star_live_ask_price_2, self.star_open_order_volume_2))
                            print("star open lower")
                            self.star_latest_price_2 = star_live_ask_price_2
                            self.star_position_open_2 = True
                            self.star_position_type_2 = 'Long'
                            self.star_remaining_quantity_2 = self.star_open_order_volume_2

                elif self.star_position_open_2:
                    if not self.star_partially_closed_2:
                        if self.star_position_type_2 == 'Long' and star_live_ask_price < star_latest_lower_band:
                            star_additional_volume_2 = min(abs(20-self.star_open_order_volume_2), abs(star_live_ask_volume))
                            star_orders.append(Order('STARFRUIT', star_live_ask_price, star_additional_volume_2))
                            self.star_remaining_quantity_2 += star_additional_volume_2
                            print("additional long position")
                            self.star_position_open_2 = True
                            self.star_position_type_2 = 'Long'
                        elif self.star_position_type_2 == 'Short' and star_live_bid_price > star_latest_upper_band:
                            star_additional_volume_2 = min(abs(20-self.star_open_order_volume_2), abs(star_live_bid_volume))
                            star_orders.append(Order('STARFRUIT', star_live_bid_price, -star_additional_volume_2))
                            self.star_remaining_quantity_2 += star_additional_volume_2
                            print("additional short position")
                            self.star_position_open_2 = True
                            self.star_position_type_2 = 'Short'
                        if self.star_position_type_2 == 'Short' and star_live_ask_price < star_latest_lower_band:
                            star_closing_volume_2 = min(abs(self.star_open_order_volume_2), abs(star_live_ask_volume))
                            star_orders.append(Order('STARFRUIT', star_live_ask_price, star_closing_volume_2))
                            self.star_remaining_quantity_2 -= star_closing_volume_2
                            if self.star_remaining_quantity_2 > 0:
                                print("star partial close upper")
                                self.star_partially_closed_2 = True
                                self.star_position_open_2 = True
                                self.star_position_type_2 = 'Short'
                            else:
                                print("star full close upper")
                                self.star_partially_closed_2 = False
                                self.star_position_open_2 = False
                        elif self.star_position_type_2 == 'Long' and star_live_bid_price > star_latest_upper_band:
                            star_closing_volume_2 = min(abs(self.star_open_order_volume_2), abs(star_live_bid_volume))
                            star_orders.append(Order('STARFRUIT', star_live_bid_price, -star_closing_volume_2))
                            self.star_remaining_quantity_2 -= star_closing_volume_2
                            if self.star_remaining_quantity_2 > 0:
                                print("star partial close lower")
                                self.star_partially_closed_2 = True
                                ...
                                self.star_partially_closed_2 = False
                                self.star_position_open_2 = False
                    elif self.star_partially_closed_2:
                        if self.star_position_type_2 == 'Short' and (star_live_ask_price < star_latest_lower_band or star_live_ask_price < self.star_latest_price_2):
                            star_order_quantity_2 = min(abs(self.star_remaining_quantity_2), abs(star_live_ask_volume))
                            star_orders.append(Order('STARFRUIT', star_live_ask_price, star_order_quantity_2))
                            self.star_remaining_quantity_2 -= star_order_quantity_2
                            if self.star_remaining_quantity_2 > 0:
                                self.star_partially_closed_2 = True
                                self.star_position_open_2 = True
                                self.star_position_type_2 = 'Short'
                                ...
                                self.star_partially_closed_2 = False
                                self.star_position_open_2 = False
                        elif self.star_position_type_2 == 'Long' and (star_live_bid_price > star_latest_upper_band or star_live_bid_price > self.star_latest_price_2):
                            star_order_quantity_2 = min(abs(self.star_remaining_quantity_2), abs(star_live_bid_volume))
                            star_orders.append(Order('STARFRUIT', star_live_bid_price, -star_order_quantity_2))
                            self.star_remaining_quantity_2 -= star_order_quantity_2
                            if self.star_remaining_quantity_2 > 0:
                                self.star_partially_closed_2 = True
                                self.star_position_open_2 = True
                                ...
                                self.star_partially_closed_2 = False
                                self.star_osition_open_2 = False
